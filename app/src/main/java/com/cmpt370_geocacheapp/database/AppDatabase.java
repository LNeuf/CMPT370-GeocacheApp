...
@Database(entities = {User.class, Geocache.class, Comment.class, RatingReview.class, Picture.class}, version = 1)
    ...
    public static AppDatabase getInstance(Context context) {
        if (dbInstance == null) {
            synchronized (AppDatabase.class) {
                if (dbInstance == null) {
                    dbInstance = Room.databaseBuilder(context.getApplicationContext(),AppDatabase.class, DATABASE_NAME).createFromAsset(DATABASE_NAME).allowMainThreadQueries().build();
